...
def summarise_agency(function, agency):
    ...
    print 'Items digitised: {} ({:.2%})'.format(total_digitised, (float(total_digitised)/total_described))
    print 'Series with no items described: {} ({:.2%})'.format(total_undescribed, (float(total_undescribed)/total_series))
    print 'Quantity undescribed: at least {} of {} metres ({:.2%})'.format(quantity_undescribed, quantity_described, (float(quantity_undescribed)/quantity_described))
